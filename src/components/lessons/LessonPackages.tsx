...
export function LessonPackages({ packages }: { packages: ServicePackage[] }) {
  const t = useTranslations("lessons");

  if (packages.length === 0) {
    return <p className="font-body text-sm text-ink/50">{t("empty")}</p>;
  }

  return (
    <div className="grid gap-4 sm:grid-cols-2">
      {packages.map((pkg) => (
        <div key={pkg.id} className="border border-ink/10 p-6">
